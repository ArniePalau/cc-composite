...
final class PackageIntegrityTest extends TestCase
{
    public function testCoreIntegrationClassesAutoload(): void
    {
        ...
        self::assertTrue(class_exists(ArmaBriefingFormatter::class));
    ...
    public function testCampaignArchiveAndManagerArePackaged(): void
    {
        ...
        self::assertStringContainsString('cc-report-card--image', file_get_contents($root . '/templates/frontend/field_report/index.html.twig'));
        self::assertStringContainsString('mask-image:linear-gradient', file_get_contents($root . '/templates/frontend/field_report/index.html.twig'));
        self::assertStringContainsString('var(--cdc-font-display', file_get_contents($root . '/templates/frontend/field_report/index.html.twig'));
        self::assertStringContainsString('cc-campaign-card--image', file_get_contents($root . '/templates/frontend/field_report/index.html.twig'));
    }
    ...
    public function testFieldReportIncludesInteractiveMapAndRichStatistics(): void
    {
        ...
        self::assertStringContainsString('frago.bodyHtml|raw', $template);
        self::assertStringContainsString('data-polyline', $template);
        self::assertStringContainsString("point.dataset.shape === 'POLYLINE'", $template);
        self::assertStringContainsString('player.distanceVehicleMeters', $template);
        self::assertStringContainsString('vehicle.distanceMeters', $template);
        self::assertStringContainsString('data.totalCivilianKills', $template);
        self::assertStringContainsString('playerFriendlyFire', $template);
        self::assertStringContainsString('cc_composite_field_reports_mission_media', $template);
        self::assertStringContainsString('cc-field__hero-art', $template);
        self::assertStringContainsString('cc-mission-gallery__header', $template);
        self::assertStringContainsString('cc-mission-gallery__grid', $template);
        self::assertStringNotContainsString('<section class="cc-mvp"', $template);
        self::assertStringNotContainsString('> Classificacions</h2>', $template);
